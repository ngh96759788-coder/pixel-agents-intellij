...
export function layoutToFurnitureInstances(furniture: PlacedFurniture[]): FurnitureInstance[] {
  ...
  for (const item of furniture) {
    ...
    const renderOffsetY = entry.renderOffsetY || 0
    const y = item.row * TILE_SIZE + renderOffsetY  // visual position (shifted)
    const footprintY = item.row * TILE_SIZE          // logical position (for depth sort)
    const spriteH = entry.sprite.length
    let zY = footprintY + spriteH
    ...
    // Wall items render in front of wall tiles (wall tiles have zY = (row+1)*TILE_SIZE)
    if (entry.canPlaceOnWalls) {
      zY = (item.row + 1) * TILE_SIZE + 0.5
    }
